...
class RunStore:
    ...
    def create_run(self, difficulty: str, client_ip: str = "") -> dict[str, str]:
        now = _utc_now()
        ...
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO runs (run_id, difficulty, created_at, expires_at, client_ip)
                VALUES (?, ?, ?, ?, ?)
                """,
                (run_id, difficulty, now.isoformat(), expires.isoformat(), client_ip),
            )
        return {
            "run_id": run_id,
    ...
    def is_run_valid(self, run_id: str, difficulty: str) -> tuple[bool, str]:
